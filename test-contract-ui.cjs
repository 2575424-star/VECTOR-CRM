...
const elements=new Map(),get=id=>{if(!elements.has(id))elements.set(id,{value:'test',textContent:'',disabled:false,checkValidity(){return true},setAttribute(){},focus(){},scrollIntoView(){},click(){downloaded++;}});return elements.get(id);};
...
(async()=>{
 get('contract_number').value='';await ctx.generateContract({preventDefault(){}});assert.match(get('contractMessage').textContent,/Номер договора/);assert.equal(downloaded,0);get('contract_number').value='test';
